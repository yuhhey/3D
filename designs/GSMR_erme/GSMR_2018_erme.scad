...
module erme_arccal_lefele(){
    inset_first_layer(inset_width=0.3, render=true)
    translate([0,0,H])rotate([0,180,0])erme();
}
...
module erme(){
    difference()
    {
        ...
        // akasztó
        translate([0,44,2.5])rotate_extrude($fn=3*12)translate([9,0,0])
            circle(r=1.5, $fn=1.5*12);
        
        translate([-68/2+4,-40/2,5])vonat_korvonal();

 ...
 */
tracklib_example(); //left
// mirror([0,1,0]) tracklib_example(); //righ //right
module tracklib_example($fn=120) {
    // Wood pieces
    //wood_track(10);
    ...
    difference(){
        union(){
            rotate([0,0,90]) wood_track_arc(110, angle=90, rails=false, $fn=120);
            translate([0,-110,0]) rotate([0,0,90]) wood_track_arc(220, angle=45, rails=false, $fn=120);
        }
     
       rotate([0,0,90]) wood_rails_arc(110, 90);
       translate([0,-110,0]) rotate([0,0,90]) wood_rails_arc(220, 45);
       translate([0,130,0]) rotate([0,0, 180]) wood_cutout();
     
       rotate(90, [0,0, 1]) translate([0,130,0])  wood_cutout();

       translate([0, -110, 0])rotate(45, [0,0, 1]) translate([0,240,0])  wood_cutout();
    }
 ...
 */
module wood_plug(solid=true,h=wood_height()) {
    ...
    union() {
        translate(v=[-$o,-post_w/2,0]) hull()
            ...
            {
                translate([0,0,1])
                    cube(size=[$o+adjusted_length,post_w,h-2]);
                translate([0,1,0])
                    cube(size=[$o+adjusted_length,post_w-2,h]);
        }
        difference() {
            translate(v=[neck_length,0,0]) {
                union() {
                    difference() {
                        hull() {
                            translate([0,0,1])
                                cylinder(h=h-2,r=wood_plug_radius());
                            cylinder(h=h,r=wood_plug_radius()-bevel_width);
                        }
                        if (!solid) {
                            translate(v=[-6,-3.2,-$o])
                                cube(size=[6,6.4,h+$o+$o]);
                            translate(v=[0,0,-$o])
                                cylinder(h=h+$o+$o,r=3.8);
                            translate(v=[-5,0,h/2+$o]) rotate([0,0,45])
                                cube(size=[7,7,h+$o+$o], center=true);
                            translate(v=[-5,0,h/2+$o]) rotate([0,0,0])
                                cube(size=[2,10,h+$o+$o], center=true);
                        }
                    }
                }
            }
 ...
 */
module wood_cutout(h=wood_height()) {
    plug_cutout(wood_plug_radius() + .3, wood_plug_neck_length(), h);
}

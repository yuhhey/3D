...
/*     CODE    */
/****************/
inset_first_layer(inset_height=0.31,inset_width=0.3, render=true){
    rotate([0,0,90])
...
module flex_track_pattern_2D(
    ...
    square([width_base + epsilon,line_thickness+epsilon]);
    ...
    translate([0, length-leftover / 2, 0])
      square([width_base,line_thickness+epsilon]);
}
...
module brio_female_2D() {
    ...
    difference() {
        ...
        translate([0, total_length - stem_length])
            circle(r = post_radius, $fn = 12*post_radius);        
    }
